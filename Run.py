...
def main(args):
    #load dataset
    hyper_source, train_loader, val_loader, test_loader, capacity, scaler = get_dataloader(args,
                                                                                           normalizer=args.normalizer,
                                                                                           single=False)
    #init model
    adj = np.loadtxt(args.adj_path, delimiter=',')
    ...
    #start training
    trainer = Trainer(model, loss, optimizer, hyper_source, train_loader, val_loader, test_loader, capacity, 
                      scaler, args, lr_scheduler=lr_scheduler, logger=args.logger, threshold=threshold)
    ...
    if args.mode == 'train':
        ...
        args.logger.info("Load saved model")
        trainer.test(model, trainer.args, hyper_source, test_loader, capacity, scaler, trainer.logger)
    elif args.mode == 'I-metrics':
        trainer.compute_I_metrics(args, capacity, trainer.logger)
    else:
        raise ValueError
...
if __name__ == "__main__":
    ...
    args.add_argument('--mask_ratio', default=config['data']['mask_ratio'], type=float)

    args.add_argument('--normalizer', default=config['data']['normalizer'], type=str)
    args.add_argument('--adj_path', default=config['data']['adj_path'], type=str)
    ...
    args.log_dir = os.path.join(current_dir, 'log', f"{args.dataset}", f"embed_{args.embed_dim}_layer_{args.num_layers}_noDyn", f"{args.exp_mode}_{args.num_clients}p_in{args.lag}_out{args.horizon}_{args.active_mode}", f"mask_ratio_{args.mask_ratio}")
