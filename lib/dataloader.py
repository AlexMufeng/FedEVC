...
from lib.add_window import Add_Window_Horizon, Add_Window_Hyper, Add_Window_Horizon_Mask
...
def get_dataloader(args, normalizer = 'std', single=False):
    #load raw st dataset
    data, capacity = load_st_dataset(args.dataset)        # T, N, 1
    #normalize st data
    ...
    x_tra, y_tra = Add_Window_Horizon_Mask(args, data_train, args.lag, args.horizon, args.mask_ratio, single)
    x_val, y_val = Add_Window_Horizon_Mask(args, data_val, args.lag, args.horizon, args.mask_ratio, single)
    x_test, y_test = Add_Window_Horizon_Mask(args, data_test, args.lag, args.horizon, args.mask_ratio, single)
    
    # X, Y = Add_Window_Horizon(args, data, args.lag, args.horizon, single)
    ...
    capacity = torch.from_numpy(capacity).to(args.device)
    return x_hyper, train_dataloader, val_dataloader, test_dataloader, capacity, scaler
