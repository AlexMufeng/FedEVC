...
class AGCRN(nn.Module):
    def __init__(self, args, adj):
        ...
        self.node_embeddings = nn.Parameter(torch.randn(self.num_node, args.embed_dim * 2), requires_grad=True).to(args.device)
        # self.node_embeddings = nn.Parameter(torch.randn(self.num_node, args.embed_dim), requires_grad=False)
        ...
        self.encoder = AVWDCRNN(self.args, adj, args.num_nodes, args.hyper_model_dim, args.input_dim, args.rnn_units,
                                args.cheb_k, args.embed_dim, self.node_embeddings[:, -args.embed_dim:], args.num_layers)
    ...
    def forward(self, hyper_source, source):
        ...
        if self.args.exp_mode in ["CTR", "SGL"]:
            ...
            weights_h = torch.einsum('nd,dhi->nhi', self.node_embeddings[:, :self.args.embed_dim], self.main_weights_pool) # N, hidden, I
        elif self.args.exp_mode == "FED":
            ...
            weights_hper = [torch.einsum('nd,dhi->nhi', self.node_embeddings[self.args.nodes_per[i], :self.args.embed_dim], self.main_weights_pool[i]) for i in range(self.args.num_clients)]
        ...
        output, _, masks, normLoss, sp, tran = self.encoder(source, init_state, self.node_embeddings[:, :self.args.embed_dim], self.poly_coefficients, h)      #B, T, N, hidden
...
class AVWDCRNN(nn.Module):
    def __init__(self, args, adj, node_num, hyper_model_dim, dim_in, dim_out, cheb_k, embed_dim, IA_emb, num_layers=1):
        ...
        self.args = args
        self.dcrnn_cells.append(AGCRNCell(self.args, adj, node_num, hyper_model_dim, self.input_dim, dim_out, cheb_k, embed_dim, IA_emb))

        for _ in range(1, num_layers):
            self.dcrnn_cells.append(AGCRNCell(self.args, adj, node_num, hyper_model_dim, dim_out, dim_out, cheb_k, embed_dim, IA_emb))
    ...
    def forward(self, x, init_state, node_embeddings, poly_coefficients, h):
        ...
        return current_inputs, output_hidden, torch.cat(mask_list, dim=-1), torch.sum(torch.stack(normLoss_list)), total_sp / (seq_length*self.num_layers), total_tran / (seq_length*self.num_layers)
